        ...
        int16_t *pTailServo)
{
    ...
    gpMixerConfig = &masterConfig.mixerConfig;
    ...
    throttleRange = masterConfig.escAndServoConfig.maxthrottle - masterConfig.escAndServoConfig.minthrottle;
...
int16_t triGetMotorCorrection(uint8_t motorIndex)
{
    ...
    if (motorIndex == TRI_TAIL_MOTOR_INDEX)
    {
        ...
        uint16_t throttleMotorOutput = tailMotorVirtual - masterConfig.escAndServoConfig.minthrottle;
...
static int16_t dynamicYaw(int16_t PIDoutput)
{
    ...
    const int16_t midpoint = masterConfig.escAndServoConfig.minthrottle + halfRange;
